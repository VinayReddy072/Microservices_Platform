...
| 5 | Defib Unit | AVAILABLE | Loan 5 REJECTED (item 1 was already `ON_LOAN`; item 5 unchanged) |
| 6 | AED Unit | ON_LOAN | `loan.approved` for loan 6 (Trace Tester, 2026-07-28) |
| 8 | Ultrasound Machine | ON_LOAN | Never loaned |

No `PUT /equipment/{id}/status` endpoint exists. The only path by which `equipment_items.status` changes is `LoanEventListener.handleLoanEvent()` in response to a RabbitMQ message. The database is not a shared integration layer.
...
- `[00:06:22]` — MySQL Workbench: two separate databases `loan_db` and `inventory_db`
- `[00:06:25]` — `loan_db.loan_requests` — no equipment-specific columns beyond `equipment_item_id`
- `[00:20:26]` — `inventory_db.equipment_items` — item 6 status changes to ON_LOAN after loan 6 approved (no REST call, only RabbitMQ)
