...
order -100               → Spring Security WebFilterChainProxy (disabled — see SecurityConfig)
```
...
During local development, domain service ports 8081 and 8082 are open on `localhost`. A client can bypass the gateway by calling `http://localhost:8081/loans` directly, bypassing JWT validation entirely. This is acceptable for the demonstration environment but would not be acceptable in production.
...
## Report Evidence — Verification Report §7
...
- `[00:XX:XX]` — Login as admin and user, show both tokens
- `[00:XX:XX]` — GET with USER token → 200; POST with USER token → 403
- `[00:XX:XX]` — POST with ADMIN token → 201; no token → 401
